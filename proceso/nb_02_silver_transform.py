...
# MAGIC %md
# MAGIC --------otras validaciones se pueden borrar
# MAGIC
...
print("=== Conteos por año en SILVER ===\n")
for tabla in ["hogar", "vivienda", "educacion", "persona"]:
    print(f"silver.{tabla}:")
    spark.table(f"ecv_dev.silver.{tabla}").groupBy("anio_encuesta").count().orderBy("anio_encuesta").show()

# COMMAND ----------

# Esta es LA validación más importante después de cargar 2018
print("=== Distribución de REGION por año ===")
spark.table("ecv_dev.silver.vivienda") \
    .groupBy("anio_encuesta", "region") \
    .count() \
    .orderBy("anio_encuesta", "region") \
    .show(50, truncate=False)

# COMMAND ----------

print("=== Estado civil por año (validar consistencia entre años) ===")
spark.table("ecv_dev.silver.hogar") \
    .groupBy("anio_encuesta", "estado_civil") \
    .count() \
    .orderBy("anio_encuesta", "count", ascending=[True, False]) \
    .show(50, truncate=False)

print("\n=== Nivel educativo por año ===")
spark.table("ecv_dev.silver.educacion") \
    .groupBy("anio_encuesta", "nivel_educativo") \
    .count() \
    .orderBy("anio_encuesta", "count", ascending=[True, False]) \
    .show(50, truncate=False)
