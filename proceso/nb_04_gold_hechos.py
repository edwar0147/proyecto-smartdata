...
try:
    ...
    claves_vivienda = ["directorio", "secuencia_encuesta", "anio_encuesta"]
    ...
    df_fact = (df_fact
        ...
        .withColumn("sk_educacion", generar_sk(claves_persona))  # mismo grano que persona
...
verificaciones = [
    ...
    ("sk_vivienda",  "dim_vivienda",  "sk_vivienda",  False),
    ("sk_educacion", "dim_educacion", "sk_educacion", False),
    ("sk_tiempo",    "dim_tiempo",    "sk_tiempo",    False),
...
# MAGIC %md
# MAGIC validacion extra se puede borrar
...
print("=" * 70)
print("INVESTIGACIÓN: ¿Por qué hay FKs sin match?")
print("=" * 70)

df_persona = spark.table("ecv_dev.silver.persona")
df_viv     = spark.table("ecv_dev.silver.vivienda")
df_edu     = spark.table("ecv_dev.silver.educacion")

# 1. Conteos generales
print(f"\nsilver.persona  : {df_persona.count():>10}")
print(f"silver.vivienda : {df_viv.count():>10}")
print(f"silver.educacion: {df_edu.count():>10}")

# 2. ¿Cuántas personas tienen directorio que NO existe en vivienda?
print("\n=== Personas cuyo (directorio, secuencia_encuesta, año) NO está en vivienda ===")
df_huerfanas_viv = df_persona.join(
    df_viv,
    (df_persona.directorio == df_viv.directorio) &
    (df_persona.secuencia_encuesta == df_viv.secuencia_encuesta) &
    (df_persona.anio_encuesta == df_viv.anio_encuesta),
    "left_anti"
)
print(f"Personas sin vivienda: {df_huerfanas_viv.count()}")
df_huerfanas_viv.groupBy("anio_encuesta").count().show()

# 3. ¿Cuántas personas NO están en educación?
print("\n=== Personas que NO tienen registro en educación ===")
df_huerfanas_edu = df_persona.join(
    df_edu,
    (df_persona.directorio == df_edu.directorio) &
    (df_persona.secuencia_encuesta == df_edu.secuencia_encuesta) &
    (df_persona.secuencia_p == df_edu.secuencia_p) &
    (df_persona.orden == df_edu.orden) &
    (df_persona.anio_encuesta == df_edu.anio_encuesta),
    "left_anti"
)
print(f"Personas sin registro educativo: {df_huerfanas_edu.count()}")

# 4. Estadística de edad para las personas sin educación
print("\n=== Edad de las personas sin registro educativo ===")
df_huerfanas_edu.select("edad").describe().show()
...
# 1. Tipos de datos de las claves
print("=== Tipos de datos en silver.persona ===")
for c in ["directorio", "secuencia_encuesta", "anio_encuesta"]:
    tipo = [f.dataType.simpleString() for f in df_persona.schema.fields if f.name == c][0]
    print(f"  {c}: {tipo}")

print("\n=== Tipos de datos en silver.vivienda ===")
for c in ["directorio", "secuencia_encuesta", "anio_encuesta"]:
    tipo = [f.dataType.simpleString() for f in df_viv.schema.fields if f.name == c][0]
    print(f"  {c}: {tipo}")

# 2. Ejemplos concretos para comparar
print("\n=== Muestra de claves en silver.persona ===")
df_persona.select("directorio", "secuencia_encuesta", "anio_encuesta") \
    .distinct().show(5)

print("\n=== Muestra de claves en silver.vivienda ===")
df_viv.select("directorio", "secuencia_encuesta", "anio_encuesta") \
    .distinct().show(5)

# 3. Tomar UN caso específico y ver si se encuentra
print("\n=== Probar con la primera persona ===")
primera = df_persona.select("directorio", "secuencia_encuesta", "anio_encuesta").first()
print(f"Buscando: directorio={primera['directorio']}, "
      f"sec_enc={primera['secuencia_encuesta']}, "
      f"año={primera['anio_encuesta']}")

coincidencias = df_viv.filter(
    (F.col("directorio") == primera['directorio']) &
    (F.col("secuencia_encuesta") == primera['secuencia_encuesta']) &
    (F.col("anio_encuesta") == primera['anio_encuesta'])
)
print(f"Coincidencias en silver.vivienda: {coincidencias.count()}")

# 4. ¿Qué secuencia_encuesta tienen las personas vs las viviendas?
print("\n=== Distribución de secuencia_encuesta en silver.persona ===")
df_persona.select("secuencia_encuesta").describe().show()

print("\n=== Distribución de secuencia_encuesta en silver.vivienda ===")
df_viv.select("secuencia_encuesta").describe().show()
